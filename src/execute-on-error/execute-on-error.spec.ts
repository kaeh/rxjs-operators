...
import { executeOnError } from "./execute-on-error";

describe(executeOnError.name, () => {
  beforeEach(() => {
  ...
  it("should notify with given function", (done) => {
    ...
    source$
      .pipe(executeOnError((err) => console.log(err)))
      // and it's subscribed
      .subscribe(
        () => {
  ...
  it("should be able to suppress error", (done) => {
    ...
    // When it's piped with operator
    source$
      .pipe(executeOnError((err) => console.log(err), { suppressErrors: true }))
      // and it's subscribed
      .subscribe(
        () => {
          // Then the console should have called it's log function once
  ...
  it("should be able to suppress error and provide a default returned value", (done) => {
    ...
    // When it's piped with operator
    source$
      .pipe(executeOnError((err) => console.log(err), { suppressErrors: true, defaultValue: 8 }))
      // and it's subscribed
      .subscribe(
        (result) => {
          // Then the console should have called it's log function once
  ...
  it("should not suppress error and provide a default value if the suppressError property is falsy", (done) => {
    ...
    // When it's piped with operator
    source$
      .pipe(executeOnError((err) => console.log(err), { defaultValue: 8 }))
      // and it's subscribed
      .subscribe(
        () => {
          throw new Error("You should never have come here");
